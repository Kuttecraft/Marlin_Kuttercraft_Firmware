...
typedef struct SettingsDataStruct {
  ...
  float mbl_z_values[TERN(MESH_BED_LEVELING, GRID_MAX_POINTS_X, 3)] [TERN(MESH_BED_LEVELING, GRID_MAX_POINTS_Y, 3)];
  ...
  #endif
  //
  // KUTTERCRAFT Variables
  //
  #if ENABLED(MENU_KUTTERCRAFT)
    bool _calibracion_on_off;
    bool _informacion_capas_coord;
    bool _inductivo_final_carrera;
    bool _tipo_de_extrusor;
    bool _guardado_on_off;
    bool _sensor_on_off;
    bool _encode_der_izd;
    bool _paso_de_varrilla;
    bool _k_melody_on_off;
    int _encoder_giro;

    int numero_de_logro_pz_;
    int numero_de_logro_km_;
...
#include "../core/debug_out.h"
  ...
  bool MarlinSettings::save() {
    ...
    //
    // KUTTERCARFT  variables
    //
    #if ENABLED(MENU_KUTTERCRAFT)
      

      EEPROM_WRITE(MarlinUI::calibracion_on_off);
      EEPROM_WRITE(MarlinUI::informacion_capas_coord);///static bool tipo_de_extrusor;
      EEPROM_WRITE(MarlinUI::inductivo_final_carrera);
      EEPROM_WRITE(MarlinUI::tipo_de_extrusor);
      EEPROM_WRITE(MarlinUI::guardado_on_off);
      EEPROM_WRITE(MarlinUI::sensor_on_off);
      EEPROM_WRITE(MarlinUI::encode_der_izd);
      EEPROM_WRITE(MarlinUI::paso_de_varrilla);
      EEPROM_WRITE(MarlinUI::k_melody_on_off);
      EEPROM_WRITE(MarlinUI::encoder_giro);//MarlinUI::encoder_giro

      EEPROM_WRITE(MarlinUI::numero_de_logro_km);
      EEPROM_WRITE(MarlinUI::numero_de_logro_pz);

      EEPROM_WRITE(MarlinUI::numero_de_serie);
    #endif
  ...
  bool MarlinSettings::_load() {
    ...
    if (strncmp(version, stored_ver, 3) != 0) {
      ...
      //
      #if ENABLED(MENU_KUTTERCRAFT)
        _FIELD_TEST(MarlinUI::calibracion_on_off);
        EEPROM_READ(MarlinUI::calibracion_on_off);

        _FIELD_TEST(MarlinUI::informacion_capas_coord);
        EEPROM_READ(MarlinUI::informacion_capas_coord);//

        _FIELD_TEST(MarlinUI::inductivo_final_carrera);
        EEPROM_READ(MarlinUI::inductivo_final_carrera);//inductivo_final_carrera

        _FIELD_TEST(MarlinUI::tipo_de_extrusor);
        EEPROM_READ(MarlinUI::tipo_de_extrusor);//tipo_de_extrusor
        
        _FIELD_TEST(MarlinUI::guardado_on_off);
        EEPROM_READ(MarlinUI::guardado_on_off);

        _FIELD_TEST(MarlinUI::sensor_on_off);
        EEPROM_READ(MarlinUI::sensor_on_off);

        _FIELD_TEST(MarlinUI::encode_der_izd);
        EEPROM_READ(MarlinUI::encode_der_izd);

        _FIELD_TEST(MarlinUI::paso_de_varrilla);
        EEPROM_READ(MarlinUI::paso_de_varrilla);

        _FIELD_TEST(MarlinUI::k_melody_on_off);
        EEPROM_READ(MarlinUI::k_melody_on_off);//encoder_giro

        _FIELD_TEST(MarlinUI::encoder_giro);
        EEPROM_READ(MarlinUI::encoder_giro);//encoder_giro

        _FIELD_TEST(MarlinUI::numero_de_logro_km);
        EEPROM_READ(MarlinUI::numero_de_logro_km);

        _FIELD_TEST(MarlinUI::numero_de_logro_pz);
        EEPROM_READ(MarlinUI::numero_de_logro_pz);

        _FIELD_TEST(MarlinUI::numero_de_serie);
        EEPROM_READ(MarlinUI::numero_de_serie);
...
void MarlinSettings::reset() {
  ...
  //
  #if ENABLED(MENU_KUTTERCRAFT)
    MarlinUI::calibracion_on_off       = true;
    MarlinUI::informacion_capas_coord  = true;
    MarlinUI::inductivo_final_carrera  = true;
    MarlinUI::tipo_de_extrusor         = true;
    MarlinUI::guardado_on_off          = true;
    MarlinUI::sensor_on_off            = false;
    MarlinUI::encode_der_izd           = true;
    MarlinUI::paso_de_varrilla         = true;
    MarlinUI::k_melody_on_off          = true;
    MarlinUI::encoder_giro             = 1;

    MarlinUI::numero_de_logro_km  = MarlinUI::numero_de_logro_km;
    MarlinUI::numero_de_logro_pz  = MarlinUI::numero_de_logro_pz;
  #endif
