...
#include "../../../inc/MarlinConfig.h"
  ...
  #include "../../../lcd/menu/menu_calibracion.h"
    ...
    if (parser.seen('P')) {
      ...
      switch (g29_phase_value) {
        ...
        case 4: // Fine Tune (i.e., Edit) the Mesh
            ...
            //MarlinUI::goto_screen(ajustar_punto_mapa);
            SERIAL_ECHOLN("esta funcion fue eliminida, no era util");
          #else
  ...
  void unified_bed_leveling::shift_mesh_height() {
    GRID_LOOP(x, y)
      ...
      void remove_Spaces_In_Place(char* str) {
    ...
    void unified_bed_leveling::probe_entire_mesh(const xy_pos_t &near, const bool do_ubl_mesh_map, const bool stow_probe, const bool do_furthest) {
      ...
      do {
        ...
        TERN_(HAS_DISPLAY, ui.status_printf_P(0, PSTR(S_FMT " %i/%i"), GET_TEXT(MSG_PROBING_MESH), point_num, int(GRID_MAX_POINTS)));
          ...
          strcpy(MarlinUI::contenedor_str, GET_TEXT(MSG_PROBING_MESH));
          strcat(MarlinUI::contenedor_str, ":");

          strcat(auxiliar_, i16tostr3rj(point_num));
          strcat(auxiliar_, "/");
          strcat(auxiliar_, i16tostr3rj(int(GRID_MAX_POINTS)));
          remove_Spaces_In_Place(auxiliar_);

          strcat(MarlinUI::contenedor_str, auxiliar_);
      ...
      //Salida al menu de espera
      #if ENABLED(MENU_KUTTERCRAFT)
        MarlinUI::salida_de_menu = true;
      #endif
    ...
    //ajustar punto de mapa kuttercraft
    int point_x,point_y = 0; //guarda el punto de matris
    float z_valor_actual = 0;
    float nueva_altura_z = 0;

    float diferencia_de_altura(float x, float y){
      return -1 * (x) + y;
    }

    void unified_bed_leveling::completar_mapa(){
      //AUTO-completa los puntos exteriores
      //con un valor creciente de la pendiente
      z_values[3][0] = z_values[2][0] + diferencia_de_altura(z_values[1][0],z_values[2][0]);
      z_values[3][1] = z_values[2][1] + diferencia_de_altura(z_values[1][1],z_values[2][1]);
      z_values[3][2] = z_values[2][2] + diferencia_de_altura(z_values[1][2],z_values[2][2]);

      z_values[0][3] = z_values[0][2] + diferencia_de_altura(z_values[0][1],z_values[0][2]);
      z_values[1][3] = z_values[1][2] + diferencia_de_altura(z_values[1][1],z_values[1][2]);
      z_values[2][3] = z_values[2][2] + diferencia_de_altura(z_values[2][1],z_values[2][2]);

      z_values[3][3] = z_values[2][2] + diferencia_de_altura(z_values[1][1],z_values[2][2]);
    }

    void unified_bed_leveling::mover_z_mapa(){
      ...
      if (ui.encoderPosition) {
        const float diff = float(int32_t(ui.encoderPosition)) * ui.manual_move.menu_scale;//incrementa en numero en 0.025 +-

        nueva_altura_z += diff; //guarda este valor

        //no puede pasar -5 o +5 mm
        if (int32_t(ui.encoderPosition) < 0)
          NOLESS(nueva_altura_z, -5);
        else
          NOMORE(nueva_altura_z, 5);
      ...
      ui.encoderPosition = 0;//reset para no contar infinita mente
      
      //dibuja el resultado
      if (ui.should_draw()){
          if(point_y == 1){
            punto_actual = 4 + point_x;
          }else if(point_y == 2){
            punto_actual = 8 + point_x;
          }else if(point_y == 3){
            punto_actual = 12 + point_x;
          }else{
            punto_actual = point_x;
          }
          strcpy(MarlinUI::contenedor_str, "Punto");
          strcat(MarlinUI::contenedor_str, ui8tostr3rj(punto_actual + 1));
          MenuEditItemBase::draw_edit_screen(MarlinUI::contenedor_str, ftostr63(nueva_altura_z));
      }
      
      //salida con un clik
      if(ui.use_click()){
        //no se mueve al punto por las dudas
        //MarlinUI::mover_a_punto = false;

        //guarda el nuevo valor en el punto correspondiente
        z_values[point_x][point_y] = nueva_altura_z;
        ...
        //salida del ajuste de altura de punto
        //no funciona para el movimiento
        //asi que hare otro if
        ui.goto_screen(ubl_map_screen_, punto_actual);
      }
    }
    
    void unified_bed_leveling::ajustar_punto_mapa(int punto_x, int punto_y) {
      //objetivo mover a 0.025
      //por los que ya que mostrar valores 0.001

      if(MarlinUI::mover_a_punto){
        //prueba de movimiento
        current_position[Z_AXIS] = 0 + z_values[punto_x][punto_y];
        ui.manual_move.soon(Z_AXIS);
      }
      
      //setea la escala de movimiento
      ...
      //se encarga de redondear el numero
      bool es_negativo = false;
      //obtienen el valor del z
      z_valor_actual = z_values[point_x][point_y];

      //si el numero es negativo lo pasa a positivo

      if(z_valor_actual <= 0){
        es_negativo = true;
        z_valor_actual = z_valor_actual * -1;
      }

      z_valor_actual = z_valor_actual * 10;//solo nos interesan los dos ultimos numero
      //variable auxiliar
      float aux_offset_re = 0;

      //valores de redondeo
      if(z_valor_actual - int(z_valor_actual) >= 0.75){
        aux_offset_re = 0.075;
      }else if(z_valor_actual - int(z_valor_actual) >= 0.50){
        aux_offset_re = 0.050;
      }else if(z_valor_actual - int(z_valor_actual) >= 0.25){
        aux_offset_re = 0.025;
      }
      //solo me quedo con el primer decimal 
      z_valor_actual = int(z_valor_actual);
      z_valor_actual = z_valor_actual / 10;//vuelve a poner el primer decimal
      // se une el numero redondeado y el resto de digitos
      z_valor_actual = z_valor_actual + aux_offset_re;

      if(es_negativo){
        z_valor_actual = z_valor_actual * -1;
      }

      nueva_altura_z = z_valor_actual;

      //va a un menu que permite la modificacion de valores
      MarlinUI::goto_screen(mover_z_mapa);
    }
