...
constexpr uint8_t epps = ENCODER_PULSES_PER_STEP;
  ...
  char MarlinUI::contenedor_str[35];
  char MarlinUI::numero_de_serie[16];
  int MarlinUI::numero_de_logro_km = 0;
  int MarlinUI::numero_de_logro_pz = 0;
  int MarlinUI::mostar_un_solo_logro = 0;
  int MarlinUI::encoder_giro = 1; 
  int MarlinUI::endcoder_ulti_aux = 0;

  char MarlinUI::capa_actual[16];
  char MarlinUI::total_de_capas[16];

  bool MarlinUI::salida_de_menu = false;

  bool MarlinUI::calibracion_on_off       =true;
  bool MarlinUI::informacion_capas_coord  =true;
  bool MarlinUI::inductivo_final_carrera  =true;
  bool MarlinUI::tipo_de_extrusor         =true;
  bool MarlinUI::guardado_on_off          =true;
  bool MarlinUI::sensor_on_off            =false;
  bool MarlinUI::encode_der_izd           =true;
  bool MarlinUI::k_melody_on_off          =true;
  bool MarlinUI::paso_de_varrilla         =true;

  bool MarlinUI::retirar_o_cargar = true;

  bool MarlinUI::salida_pid = false;

  bool MarlinUI::mover_a_punto = false;
...
// Encoder Handling
  ...
  bool MarlinUI::screen_changed;
    ...
    int8_t MarlinUI::encoderDirection = MarlinUI::encoder_giro;
  #endif
  ...
  #endif
  #if ENABLED(MENU_KUTTERCRAFT)
    void MarlinUI::sonar_kuttermelody(){
      //cancion de finalizacion y logros
      //duracion y frecuencia
      //Solo suena si esta activa la melodia
  ...
  void MarlinUI::sonar_kutterajustes(){
    if(MarlinUI::k_melody_on_off){
        buzz(125, 1174);
        buzz(125, 1174);
        buzz(125, 2349);
...
////////////////////////////////////////////
  ...
  bool MarlinUI::handle_keypad() {
      ...
      if (keypad_buttons) {
          ...
          #if HAS_LCD_MENU
            if (encoderDirection == -(MarlinUI::encoder_giro)) { // ADC_KEYPAD forces REVERSE_MENU_DIRECTION, so this indicates menu navigation
...
////////////////////////////////////////////
  ...
  #if ENABLED(AUTO_BED_LEVELING_UBL) || ENABLED(MENU_KUTTERCRAFT)
...
void MarlinUI::update() {
  ...
  if (ELAPSED(ms, next_lcd_update_ms) || TERN0(HAS_MARLINUI_U8GLIB, drawing_screen)) {
    ...
    #endif
    //se encarda de preguntar si hay un nuevo logro con un delay casi 10 segundos
  ...
  void MarlinUI::update_buttons() {
    ...
    if (ELAPSED(now, next_button_update_ms)) {
      ...
      if (enc != lastEncoderBits) {
        ...
        #if BOTH(HAS_LCD_MENU, AUTO_BED_LEVELING_UBL) || ENABLED(MENU_KUTTERCRAFT)
          external_encoder();
        #endif
        lastEncoderBits = enc;
      }
