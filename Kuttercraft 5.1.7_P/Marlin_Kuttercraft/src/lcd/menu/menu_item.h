...
// SUBMENU(LABEL, screen_handler)
class MenuItem_submenu2 : public MenuItemBase {
  public:
    FORCE_INLINE static void draw(const bool sel, const uint8_t row, PGM_P const pstr, ...) {
      _draw(sel, row, pstr, '>', ']');
    }
...
#define NEXT_ITEM_(NUM) (_thisItemNr = NUM)
...
#define SUBMENU2_N_S_P(N, S, PLABEL, DEST)            MENU_ITEM_N_S_P(submenu2, N, S, PLABEL, DEST)
#define SUBMENU2_N_S(N, S, LABEL, DEST)                 SUBMENU2_N_S_P(N, S, GET_TEXT(LABEL), DEST)
#define SUBMENU2_S_P(S, PLABEL, DEST)                   MENU_ITEM_S_P(submenu2, S, PLABEL, DEST)
#define SUBMENU2_S(S, LABEL, DEST)                        SUBMENU2_S_P(S, GET_TEXT(LABEL), DEST)
#define SUBMENU2_N_P(N, PLABEL, DEST)                   MENU_ITEM_N_P(submenu2, N, PLABEL, DEST)
#define SUBMENU2_N(N, LABEL, DEST)                        SUBMENU2_N_P(N, GET_TEXT(LABEL), DEST)
#define SUBMENU2_P(PLABEL, DEST)                          MENU_ITEM_P(submenu2, PLABEL, DEST)
#define SUBMENU2(LABEL, DEST)                               SUBMENU2_P(GET_TEXT(LABEL), DEST)
