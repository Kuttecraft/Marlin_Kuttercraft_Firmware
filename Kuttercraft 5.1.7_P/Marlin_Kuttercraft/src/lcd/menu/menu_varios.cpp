...
void espera_cancelacion_posicion(){
    ...
    //espera a estar en posicion a menos que se venga de una pausa
    if((lpos.x == 5 && lpos.y == 5 && contador_de_vueltas >= 10) || modo_pausa){
        //ocurre un bug al setear la temperatura directamente
        //por eso esperamos
        //esperamos a su ejecucion
        //secuencia_ejecucion
        ...
        if(secuencia_ejecucion == 0){
            if(!queue.has_commands_queued()){
                ...
                PrintCounter::data.finishedPrints -= 1;
                PrintCounter::saveStats();
                queue.inject_P(PSTR("M117 Impresion Cancelada"));
            }
        }

        //establece la temperatura del hotend
        if(secuencia_ejecucion == 1){
            if(!queue.has_commands_queued()){
                ...
                queue.inject_P(PSTR(MarlinUI::contenedor_str)); //M104
            }
        }
        ...
        if(secuencia_ejecucion == 2){
            if(!queue.has_commands_queued()){
                ...
                queue.inject_P(PSTR(MarlinUI::contenedor_str)); //M140
            }
        }
...
void espera_de_cancelacion(){
    ...
    if(queue.enqueue_one_P(PSTR("G1 X5 Y5 F1500"))){
