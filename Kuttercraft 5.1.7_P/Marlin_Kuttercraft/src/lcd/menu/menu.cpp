...
////////////////////////////////////////////
  ...
  int MarlinUI::ultimo_valor_encoder = 0;
  screenFunc_t MarlinUI::ultimo_valor_ventana = status_screen;
#endif
...
void MarlinUI::save_previous_screen() {
  //salida aux
  #if ENABLED(MENU_KUTTERCRAFT)
    MarlinUI::endcoder_ulti_aux = encoderPosition;
    //SERIAL_ECHOLN("Se Guardo Encoder:");
    //SERIAL_ECHOLN(MarlinUI::endcoder_ulti_aux);
  #endif
...
void MarlinUI::_goto_previous_screen(TERN_(TURBO_BACK_MENU_ITEM, const bool is_back/*=false*/)) {
  #if ENABLED(MENU_KUTTERCRAFT)
    goto_screen(ultimo_valor_ventana, ultimo_valor_encoder);
  #else
  ...
  #include "../../module/settings.h"
    ...
    void lcd_babystep_zoffset() {
      ...
      if (ui.encoderPosition) {
        ...
        const float diff =  float(int32_t(ui.encoderPosition)) * 0.025;

        const float new_probe_offset = probe.offset.z + diff;

        const float new_offs = TERN(BABYSTEP_HOTEND_Z_OFFSET
        ...
        if (WITHIN(new_offs, Z_PROBE_OFFSET_RANGE_MIN, Z_PROBE_OFFSET_RANGE_MAX)) {
          ...
          babystep.add_steps(Z_AXIS, diff);
