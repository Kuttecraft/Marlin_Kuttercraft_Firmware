...
FORCE_INLINE void _draw_centered_temp(const int16_t temp, const uint8_t tx, const uint8_t ty) {
  ...
  FORCE_INLINE void _draw_axis_value(const AxisEnum axis, const char *value, const bool blink) {
    ...
    lcd_put_wchar(X_LABEL_POS + offs, 63, axis_codes[axis]);
    lcd_moveto(X_VALUE_POS + offs, 63);
    if (blink)
      lcd_put_u8str(value);
    else {
...
*/
#else
  //dibuja el status screen del menu kuttercraft
  void MarlinUI::draw_status_screen() {

    static char xstring[TERN(LCD_SHOW_E_TOTAL, 12, 5)], ystring[5], zstring[8];

    #define PROGRESS_BAR_Y 41
    #define PROGRESS_BAR_X 1
    #define PROGRESS_BAR_WIDTH (LCD_PIXEL_WIDTH - PROGRESS_BAR_X) -  1
    ...
    u8g.drawBitmapP(39, 13, 3, 18,
        blink && thermalManager.fan_speed[0] ? info_fan_0 : info_fan_1
    );

    if (blink && thermalManager.isHeatingHotend(0)) {
    ...
    if(blink && thermalManager.isHeatingBed()) {
    ...
    //
      ///////////////////////////
      //Mostrar temperatura cama
      lcd_moveto(93, 17);
      lcd_put_u8str(i16tostr4signrj(int(thermalManager.degTargetBed())));
      lcd_put_wchar(LCD_STR_DEGREE[0]);

      lcd_moveto(93, 39);
      lcd_put_u8str(i16tostr4signrj(int(thermalManager.degBed())));
      lcd_put_wchar(LCD_STR_DEGREE[0]);

      /////////////////////////////
      //Mostrar temperatura hotend
      lcd_moveto(62, 17);
      lcd_put_u8str(i16tostr4signrj(int(thermalManager.degTargetHotend(0))));
      lcd_put_wchar(LCD_STR_DEGREE[0]);

      lcd_moveto(62, 39);
      lcd_put_u8str(i16tostr4signrj(int(thermalManager.degHotend(0))));
      lcd_put_wchar(LCD_STR_DEGREE[0]);
      ...
      /////////////
      //Mostrar fan
      int fanSpeed = int((thermalManager.fan_speed[0] * 100) / 255);
      ...
      lcd_moveto(37, 39);
      lcd_put_u8str(i16tostr3rj(fanSpeed));
      lcd_put_wchar('%');
      ...
      if (PAGE_CONTAINS(EXTRAS_BASELINE - INFO_FONT_ASCENT, EXTRAS_BASELINE - 1)) {
        ...
        #if ENABLED(DOGM_SD_PERCENT)
          lcd_moveto(6, 54);   // posicion del texto
          if (progress_string[0]) {
            lcd_put_wchar('P');
            lcd_put_u8str(progress_string); // Percent complete
            lcd_put_wchar('%');
          }else{
             lcd_put_u8str_P(PSTR("P 00%"));
          }
        #endif
        
        lcd_moveto(44, 54);

        if(elapsed_string[0] == '\0'){
          lcd_put_u8str_P(PSTR("T00:00"));
    ...
    #define XYZ_FRAME_TOP 55
    #define XYZ_FRAME_HEIGHT 10

    if(MarlinUI::informacion_capas_coord){//muestra las capas
      //MarlinUI::total_de_capas
      //MarlinUI::capa_actual
      u8g.drawBox(0, XYZ_FRAME_TOP, LCD_PIXEL_WIDTH, XYZ_FRAME_HEIGHT);

      u8g.setColorIndex(0);
      lcd_moveto(1, 63);
      lcd_put_u8str("Capas:");
      lcd_put_u8str(MarlinUI::capa_actual);
      lcd_put_wchar('/');
      lcd_put_u8str(MarlinUI::total_de_capas);
      u8g.setColorIndex(1);

    }else{//muestra las cordenadas
      if (PAGE_CONTAINS(XYZ_FRAME_TOP, XYZ_FRAME_TOP + XYZ_FRAME_HEIGHT - 1)) {

        u8g.drawBox(0, XYZ_FRAME_TOP, LCD_PIXEL_WIDTH, XYZ_FRAME_HEIGHT);   //Caja Mostrar ejes
    ...
    //muestra los mensajes de informacion
    u8g.drawBox(0, 0, 128, 9); //Marco superior
    draw_status_message(blink);

    lcd_moveto(elapsed_x_pos, estimation_x_pos);
