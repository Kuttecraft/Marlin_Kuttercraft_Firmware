...
void _draw_bootscreen_bmp(int bitmap) {
  
  if(bitmap >= 0){
    u8g.drawBitmapP(1,1,4,30,logo_kuttercraft_00);  //logo_kuttercraft_01
  }
  if(bitmap >= 1){
    u8g.drawBitmapP(27,8,2,16,logo_kuttercraft_01); //k
    if(bitmap == 1)u8g.drawVLine(39,8,16);
  }
  if(bitmap >= 2){
    u8g.drawBitmapP(38,8,1,16,logo_kuttercraft_02); //u
    if(bitmap == 2)u8g.drawVLine(46,8,16);
  }
  if(bitmap >= 3){
    u8g.drawBitmapP(46,8,2,16,logo_kuttercraft_03); //t
    if(bitmap == 3)u8g.drawVLine(55,8,16);
  }
  if(bitmap >= 4){
    u8g.drawBitmapP(55,8,2,16,logo_kuttercraft_03); //t
    if(bitmap == 4)u8g.drawVLine(64,8,16);
  }
  if(bitmap >= 5){
    u8g.drawBitmapP(64,8,1,16,logo_kuttercraft_04); //e
    if(bitmap == 5)u8g.drawVLine(72,8,16);
  }
  if(bitmap >= 6){
    u8g.drawBitmapP(72,8,1,16,logo_kuttercraft_05); //r
    if(bitmap == 6)u8g.drawVLine(80,8,16);
  }
  if(bitmap >= 7){
    u8g.drawBitmapP(80,8,1,16,logo_kuttercraft_06); //c
    if(bitmap == 7)u8g.drawVLine(88,8,16);
  }
  if(bitmap >= 8){
    u8g.drawBitmapP(88,8,1,16,logo_kuttercraft_05); //r
    if(bitmap == 8)u8g.drawVLine(96,8,16);
  }
  if(bitmap >= 9){
    u8g.drawBitmapP(96,8,1,16,logo_kuttercraft_07); //a
    if(bitmap == 9)u8g.drawVLine(104,8,16);
  }
  if(bitmap >= 10){
    u8g.drawBitmapP(104,8,1,16,logo_kuttercraft_08);//t
    if(bitmap == 10)u8g.drawVLine(112,8,16);
  }
  if(bitmap >= 11){
    u8g.drawBitmapP(112,8,2,16,logo_kuttercraft_03);//f
    u8g.drawVLine(121,8,16);
  }

  if(bitmap >= 12){
    u8g.setFont(u8g_font_4x6);
    lcd_moveto(27, 30);   // posicion del texto
    lcd_put_u8str_P(PSTR("IMPRESORAS 3D")); // el texto
    u8g.drawHLine(16,30,63);
    u8g.drawVLine(79,23,8);
  }

  if(bitmap >= 13){
    lcd_put_u8str_P(PSTR(" 32Bits"));
    u8g.drawBox(0, 55, 128, 9);
    u8g.setFont(u8g_font_6x10);
    lcd_moveto(7, 63);
    u8g.setColorIndex(0);
    lcd_put_u8str_P(PSTR("INDUSTRIA ARGENTINA"));
    u8g.setColorIndex(1);
  }
  if(bitmap >= 14){
    lcd_moveto(18, 41);
    lcd_put_u8str_P(PSTR("Versión:"));
    lcd_put_u8str_P(PSTR(SHORT_BUILD_VERSION));

    lcd_moveto(24, 51);
    lcd_put_u8str_P(PSTR("Modelo:"));//MODELO_DE_MAQUINA
...
/*
auto _draw_bootscreen_bmp(int bitmap) {
      
      u8g.drawBitmapP(1,1,4,30,logo_kuttercraft_00);  //logo_kuttercraft_01
      
      u8g.drawBitmapP(27,8,2,16,logo_kuttercraft_01); //k
      //u8g.drawVLine(39,8,16);
      u8g.drawBitmapP(38,8,1,16,logo_kuttercraft_02); //u
      //u8g.drawVLine(46,8,16);
      u8g.drawBitmapP(46,8,2,16,logo_kuttercraft_03); //t
      //u8g.drawVLine(55,8,16);
      u8g.drawBitmapP(55,8,2,16,logo_kuttercraft_03); //t
      //u8g.drawVLine(64,8,16);
      u8g.drawBitmapP(64,8,1,16,logo_kuttercraft_04); //e
      //u8g.drawVLine(64,8,16);
      u8g.drawBitmapP(72,8,1,16,logo_kuttercraft_05); //r
      //u8g.drawVLine(80,8,16);
      u8g.drawBitmapP(80,8,1,16,logo_kuttercraft_06); //c
      //u8g.drawVLine(88,8,16);
      u8g.drawBitmapP(88,8,1,16,logo_kuttercraft_05); //r
      //u8g.drawVLine(96,8,16);
      u8g.drawBitmapP(96,8,1,16,logo_kuttercraft_07); //a
      //u8g.drawVLine(104,8,16);
      u8g.drawBitmapP(104,8,2,16,logo_kuttercraft_03);//t
      //u8g.drawVLine(113,8,16);
      u8g.drawBitmapP(113,8,1,16,logo_kuttercraft_08);//f
      u8g.drawVLine(121,8,16);
      ...
      u8g.drawBox(0, 55, 128, 9); //Marco superior
      ...
      u8g.setColorIndex(0);
      u8g.drawBox(bitmap, 0, 128 - bitmap, 31);
      u8g.setColorIndex(1);
    
    }
*/
void draw_bootscreen_bmp(int bitmap) {
...
bool MarlinUI::detected() { return true; }
  ...
  void MarlinUI::draw_marlin_bootscreen(const bool line2/*=false*/) {
    ...
    */
    ...
    #if ENABLED(MENU_KUTTERCRAFT)
      constexpr millis_t d = 70;//dalay

      LOOP_L_N(f, 15) {
        draw_bootscreen_bmp(f);
        if (d) safe_delay(d);
      }
...
void MarlinUI::clear_lcd() { } // Automatically cleared by Picture Loop
  ...
  void MenuItem_confirm::draw_select_screen(PGM_P const yes, PGM_P const no, const bool yesno, PGM_P const pref, const char * const string/*=nullptr*/, PGM_P const suff/*=nullptr*/) {
    ...
    void MenuItem_sdbase::draw(const bool sel, const uint8_t row, PGM_P const, CardReader &theCard, const bool isDir) {
       ...
       */
      #define MAP_UPPER_LEFT_CORNER_X 39  // These probably should be moved to the .h file  But for now,
      #define MAP_UPPER_LEFT_CORNER_Y  0  // it is easier to play with things having them here
      #define MAP_MAX_PIXELS_X        51
      #define MAP_MAX_PIXELS_Y        47
      ...
      void MarlinUI::ubl_plot(const uint8_t x_plot, const uint8_t y_plot) {
        ...
        if(int32_t(ui.encoderPosition) == -1){
          u8g.drawBitmapP(9, 11, 3, 23, boton_volver_b);
        }else{
          u8g.drawBitmapP(9, 11, 3, 23, boton_volver_a);
        }
        ...
        if (PAGE_CONTAINS(LCD_PIXEL_HEIGHT - (INFO_FONT_HEIGHT - 1), LCD_PIXEL_HEIGHT)) {
          ...
          if(int32_t(ui.encoderPosition) != -1){
            ...
            lcd_put_u8str_P(ftostr52(lpos.x));

            lcd_put_u8str_P(PSTR(" / "));

            lcd_put_u8str_P(Y_LBL);
            lcd_put_u8str_P(ftostr52(lpos.y));
            //X:045.00 / Y:165.00

            //(0,0) / Z: 0.089
            lcd_put_wchar(25, LCD_PIXEL_HEIGHT - 1, '(');
            u8g.print(x_plot);
            lcd_put_wchar(',');
            u8g.print(y_plot);
            lcd_put_wchar(')');
